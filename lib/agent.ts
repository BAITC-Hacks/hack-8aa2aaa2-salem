...
export type AgentReply={text:string;mode:'agent';products:Product[];steps:{label:string;ok:boolean}[];suggestions:string[];comparison?:Product[];proposal?:unknown;cartLink?:string;sources?:{title:string;url:string;checkedAt:string}[];solution?:SolutionPlan};
